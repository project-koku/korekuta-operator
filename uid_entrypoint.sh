...
if ! whoami &> /dev/null; then
    if [ -w /etc/passwd ]; then
        ...
        echo "${USER_NAME:-ansible}:x:$(id -u):0:${USER_NAME:-runner} user:${HOME}:/sbin/nologin" >> /tmp/passwd
...
exec tini -- /usr/local/bin/ansible-operator exec-entrypoint ansible  --watches-file=/opt/ansible/watches.yaml "$@"
